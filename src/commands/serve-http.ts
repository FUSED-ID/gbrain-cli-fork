...
<div class="prompt">&ldquo;Give me the GBrain admin login link&rdquo;</div>
  ...
  app.post('/mcp', requireBearerAuth({ verifier: oauthProvider }), async (req: Request, res: Response) => {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      // MERGE-INTENT §3.5: use inline-computed takesHoldersAllowList (DB lookup above)
      // rather than authInfo.takesHoldersAllowList which requireBearerAuth never populates.
